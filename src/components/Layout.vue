        ...
        methods: {
            click: function (data) {
                if (data.type == 2) {
                    if (data.link) {
                        console.log('test')
                        this.$axios.$get('/locker/' + data.link)
                            .then(res => {
                                console.log(res,'testtest')
                                store.commit('LockerData', res);
                            })
                            .catch(err => {
                                console.log(err)
                            })
        ...
        created() {
            this.$axios.$get('/locker/' + this.datas.id)
                .then(res => {
                    ...
                    let itemsIdx = 0;
                    for (let responseIdx = 0; responseIdx < response.length; responseIdx++) {
                        if (response[responseIdx].type == 6) {
                            itemsIdx++;
                            this.items[itemsIdx] = [];
                            continue;
                        }
                        this.items[itemsIdx].push(response[responseIdx])
                    }
                    this.$store.commit('LockerData', this.items);
                })
                .catch(err => {
                    console.log(err)
                })
